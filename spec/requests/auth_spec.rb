...
RSpec.describe "API_V1::Auth", :type => :request do
	...
	example "register" do
		post "/api/v1/signup", params: { :email => "foobar@example.com", :password => "123456" }
		expect(response).to have_http_status(200)

		new_user = User.last
		expect(new_user.email).to eq("foobar@example.com")

		expect(response.body).to eq( { :user_id => new_user.id }.to_json )
	end

	example "register failed" do
		post "/api/v1/signup", params: { :email => "foobar@example.com" }
	...
	before do
		@user = User.create!( :email => "foobar2@example.com", :password => "123456" )
	end
